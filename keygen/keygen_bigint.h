...
void BigInt_FromString(const char *string, int base, BigInt target);
...
void BigInt_FromString(const char *string, int base, BigInt target)
{
    if(base==16)
    {
        ...
        if(target->digits)
            free(target->digits);
        target->digits=(DIGIT*)malloc(len*sizeof(DIGIT));
        memcpy(target->digits, temp3, len);
    }
    else
    {
        ...
        while(*c)
        {
            ...
            {
                BigInt_Set(t, *c-'0');
                BigInt_Multiply(target, base_, t2);
